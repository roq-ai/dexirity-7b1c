...
import { getTokenById } from 'apiSdk/tokens';
...
function TokenViewPage() {
  ...
  const { data, error, isLoading, mutate } = useSWR<TokenInterface>(
    ...
    () =>
      getTokenById(id, {
        relations: ['organization', 'transaction'],
      }),
  );
      ...
      <Box bg="white" p={4} rounded="md" shadow="md">
        <Flex justifyContent="space-between" mb={4}>
          ...
          {hasAccess('token', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
            <NextLink href={`/tokens/edit/${data?.id}`} passHref legacyBehavior>
              <Button
              ...
              {hasAccess('transaction', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                <Stack spacing={2} mb={8}>
                  <Flex justifyContent="space-between">
                    ...
                    <NextLink passHref href={`/transactions/create?token_id=${data?.id}`}>
